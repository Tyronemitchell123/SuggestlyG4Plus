...
# This script safely clears WhatsApp Desktop backup data to free up storage space
...
Write-Host "`n📊 Current Storage Usage:" -ForegroundColor Yellow
...
Write-Host "`n⚠️  WARNING: This will delete WhatsApp Desktop backup data!" -ForegroundColor Red
...
Write-Host "`n🔄 Closing WhatsApp Desktop if running..." -ForegroundColor Yellow
...
if ($whatsappProcesses) {
    ...
    Write-Host "ℹ️  WhatsApp Desktop is not running." -ForegroundColor Blue
}
...
function Remove-WhatsAppFiles {
    ...
    if (Test-Path $Path) {
        try {
            Write-Host "🗑️  Clearing $Description..." -ForegroundColor Yellow
        ...
        Write-Host "ℹ️  $Description not found." -ForegroundColor Blue
        return 0
    }
}
...
if (Test-Path $sharedPath) {
    Write-Host "🗑️  Clearing media files from shared folder..." -ForegroundColor Yellow
    try {
        ...
        if ($mediaFiles) {
            ...
            Write-Host "ℹ️  No media files found to clear." -ForegroundColor Blue
        }
    }
    catch {
...
Write-Host "`n✅ You can now restart WhatsApp Desktop!" -ForegroundColor Green
